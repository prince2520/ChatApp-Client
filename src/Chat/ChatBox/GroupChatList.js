        ...
        <div className="box">
            <div className="friend_profile">
                ...
                <div style={{
                    ...
                    <img style={{borderRadius: '50%', width: '80%', height: '80%'}}
                         src={`${selectedGroupData.groupImageUrl}`} alt=""/>
                </div>
            ...
            <ul className="lists">
                {groupMessage.map(msg => <li className={`${msg.userName !== authctx.userName ? '' : 'list_personal'}`}
                    ...
                    {msg.userName !== authctx.userName &&
                    <p style={{fontSize: '0.7rem', alignSelf: 'center'}}> {msg.userName}</p>}

                </li>)}
                <div ref={messageEnd}/>
            </ul>}
            <input value={`${message}`}
                   onChange={(event) => dispatch(groupChatActions.sendMessageHandler({message: event.target.value}))}
